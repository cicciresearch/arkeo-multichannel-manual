Every measurement start with a JV scan to obtain the device's key parameters such as Voc, Jsc and MPP. If tracking is enabled, a JV scan is repeated a fixed intervals to obtain these parameters over time.
...
- **Voc**: The voltage at which current is 0 A.
- **Jsc**: The current at which voltage is 0 V.
- **V_MPP**: The voltage at the MPP
- **J_MPP**: The current at the MPP
